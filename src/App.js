...
// Import các thư viện Capacitor
import { Capacitor } from '@capacitor/core';
import { GoogleAuth } from '@codetrix-studio/capacitor-google-auth';


// --- Helper Functions & SVG Icons (Không thay đổi) ---
...
const WithdrawalModal = ({ onClose, user, userCoins, showNotification, onWithdrawalSuccess, backendUrl }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!amount || withdrawalAmount <= 0) { showNotification("Vui lòng nhập số xu hợp lệ.", 'error'); return; }
        if (withdrawalAmount > userCoins) { showNotification("Số xu không đủ để rút!", 'error'); return; }
        if (withdrawalAmount < 2000) { showNotification("Số tiền rút tối thiểu là 2,000 xu.", 'error'); return; }
        setIsLoading(true);
        try {
            const response = await fetch(`${backendUrl}/api/withdraw`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ bank, accountNumber, accountName, amount: withdrawalAmount, userId: user.id }),
            });
            ...
            onWithdrawalSuccess(userCoins - withdrawalAmount);
            onClose();
        } catch (error) {
    ...
    return (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-40" onClick={onClose}><div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-sm mx-4" onClick={e => e.stopPropagation()}><h3 className="text-xl font-bold text-center mb-4">Rút tiền</h3><div className="bg-gray-100 p-3 rounded-lg mb-4 text-sm text-gray-700"><h4 className="font-bold mb-2">Thể lệ rút tiền:</h4><ul className="list-disc list-inside space-y-1"><li>Số xu rút tối thiểu: <span className="font-bold">2,000 xu</span>.</li><li>Tỷ giá quy đổi: <span className="font-bold">100 xu = 1,000đ</span>.</li><li>Thời gian xử lý: <span className="font-bold">1-3 ngày</span> làm việc.</li><li className="mt-2"><span className="font-bold text-red-600">Lưu ý:</span> Kiểm tra kỹ thông tin trước khi rút. Chúng tôi không chịu trách nhiệm cho các trường hợp nhập sai thông tin.</li></ul></div><form onSubmit={handleSubmit} className="space-y-4"><input type="text" placeholder="Ngân hàng (VD: Vietcombank)" className="w-full p-2 border rounded" required value={bank} onChange={e => setBank(e.target.value)} /><input type="text" placeholder="Số tài khoản" className="w-full p-2 border rounded" required value={accountNumber} onChange={e => setAccountNumber(e.target.value)} /><input type="text" placeholder="Tên chủ tài khoản" className="w-full p-2 border rounded" required value={accountName} onChange={e => setAccountName(e.target.value)} /><div><input type="number" placeholder="Số xu muốn rút" value={amount} onChange={e => setAmount(e.target.value)} className="w-full p-2 border rounded" required /><p className="text-xs text-gray-500 mt-1">Số dư của bạn: {userCoins} xu</p></div><button type="submit" className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-2 rounded-lg transition disabled:bg-gray-400" disabled={isLoading}>{isLoading ? 'Đang gửi...' : 'Xác nhận rút tiền'}</button></form></div></div>);
};
...
const LoginPage = ({ onLogin, showNotification, backendUrl }) => {
    ...
    const handleAuthAction = async () => {
        ...
        try {
            ...
            if (isLoginView) {
                ...
                if (!response.ok || !result.success) { throw new Error(result.message || 'Đăng nhập thất bại.'); }
                ...
                result = await response.json();
                if (!response.ok || !result.success) { throw new Error(result.message || 'Đăng ký thất bại.'); }
                showNotification(result.message, 'success');
                setIsLoginView(true);
            }
        } catch (error) {
    ...
    const handleNativeGoogleLogin = async () => {
        ...
        try {
            ...
            if (googleUser && googleUser.authentication?.idToken) {
                handleGoogleSuccess(googleUser.authentication.idToken);
            } else {
                throw new Error("Không nhận được thông tin người dùng từ Google.");
            }
        } catch (err) {
            console.error("Lỗi đăng nhập Google native:", err);
    ...
    return (<div className="min-h-screen bg-gray-100 flex flex-col justify-center items-center p-4"><div className="w-full max-w-sm bg-white p-8 rounded-2xl shadow-lg text-center"><h1 className="text-3xl font-bold text-gray-800 mb-2">{isLoginView ? 'Chào mừng!' : 'Tạo tài khoản'}</h1><p className="text-gray-500 mb-8">{isLoginView ? 'Đăng nhập để săn sale và tích xu.' : 'Điền thông tin để bắt đầu.'}</p><form onSubmit={(e) => { e.preventDefault(); handleAuthAction(); }}><div className="space-y-4"><input type="text" placeholder="Tên đăng nhập" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value={username} onChange={e => setUsername(e.target.value)} required /><input type="password" placeholder="Mật khẩu" className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value={password} onChange={e => setPassword(e.target.value)} required />{!isLoginView && (<><input type="password" placeholder="Nhập lại mật khẩu" className="w-full px-4 py-3 border border-gray-300 rounded-lg" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required /><input type="text" placeholder="Mã giới thiệu (nếu có)" value={referralCode} onChange={e => setReferralCode(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg" /></>)}</div><button type="submit" className="w-full mt-6 bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-4 rounded-lg transition-transform transform hover:scale-105 disabled:bg-gray-400" disabled={isLoading}>{isLoading ? 'Đang xử lý...' : (isLoginView ? 'Đăng nhập' : 'Đăng ký')}</button></form><div className="my-6 flex items-center"><div className="flex-grow border-t border-gray-300"></div><span className="flex-shrink mx-4 text-gray-400">Hoặc</span><div className="flex-grow border-t border-gray-300"></div></div>
        ...
        <p className="mt-8 text-sm">{isLoginView ? 'Chưa có tài khoản? ' : 'Đã có tài khoản? '}<button onClick={() => setIsLoginView(!isLoginView)} className="font-semibold text-blue-500 hover:underline">{isLoginView ? 'Đăng ký ngay' : 'Đăng nhập'}</button></p></div></div>);
};
...
const ProfilePage = ({ user, coins, onDailyLogin, onWatchAd, onLogout, canClaimDailyBonus, onWithdraw, onShareReferral, isAdLoading }) => {
        ...
        <div className="p-3 text-center">
            ...
            <div className="space-y-3">
                ...
                <button 
                    ...
                    className="w-full bg-purple-500 text-white font-bold py-3 rounded-lg disabled:bg-gray-400 disabled:cursor-not-allowed"
                >
                    {isAdLoading ? 'Đang xử lý...' : 'Xem quảng cáo (+5 xu)'}
                </button>
            </div>
            <div className="bg-white p-4 rounded-lg shadow-sm mt-6 text-left">
                ...
                <p className="text-sm text-gray-600 mb-3">Khi mua hàng thành công qua các liên kết trong app, bạn sẽ nhận được số xu thưởng tương ứng với từng sản phẩm. Số xu được tính dựa trên giá trị đơn hàng và tỷ lệ hoa hồng từ nhà bán hàng.</p>
            </div>
            ...
            <div className="p-3 mt-4">
                ...
                <div className="text-center mt-6">
                    ...
                    {!isLoadingMore && hasMore && (
                        <button onClick={loadMore} className="bg-white border border-gray-300 text-gray-700 font-semibold py-2 px-6 rounded-full shadow-sm hover:bg-gray-50 transition">
                            Xem thêm
                        </button>
                    )}
...
export default function App() {
    ...
    useEffect(() => {
        if (Capacitor.isNativePlatform()) {
            // Plugin của Codetrix không cần tùy chọn khi initialize
    ...
    const handleLogin = (userData) => {
        ...
        setCanClaimBonus(userData.canClaimBonus);
    ...
    const handleDailyLogin = async () => {
        ...
        try {
            ...
            console.error("Lỗi khi nhận thưởng hàng ngày:", error);
    ...
    const handleAdReward = async () => {
        ...
        try {
            ...
            const response = await fetch(`${backendUrl}/api/user/add-coins`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId: currentUser.id, amountToAdd: 5 }),
            });
            ...
            setCoins(result.newCoins);
            showNotification("Bạn đã được cộng 5 xu!", 'success');

        } catch (error) {
            console.error("Lỗi khi cộng xu xem quảng cáo:", error);
            showNotification(error.message, 'error');
        } finally {
            setIsAdLoading(false);
        }
    ...
    const handleShareReferral = async (code) => {
        const shareData = {
            ...
            url: "https://play.google.com/store/apps/details?id=com.hunglemmo.sanvoucher",
        };
        try {
        ...
        switch (currentPage) {
            ...
            case 'profile':
                ...
                return <ProfilePage 
                    ...
                    onWithdraw={() => setIsWithdrawalVisible(true)} 
    ...
    return (
        <GoogleOAuthProvider clientId={GOOGLE_CLIENT_ID}>
            <div className="bg-gray-100 min-h-screen font-sans">
                ...
                {isWithdrawalVisible && <WithdrawalModal 
                    onClose={() => setIsWithdrawalVisible(false)} 
                    user={currentUser} 
                    userCoins={coins} 
                    showNotification={showNotification} 
                    onWithdrawalSuccess={(newCoinTotal) => setCoins(newCoinTotal)} 
