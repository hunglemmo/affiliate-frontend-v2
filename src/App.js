        ...
        <div className="p-4">
            ...
            {isLoading ? <p className="text-center">Đang tải...</p> : (
                <div className="space-y-4">
                    {history.length === 0 ? <p className="text-center text-gray-500">Bạn chưa có lịch sử đổi thưởng nào.</p> :
                        history.map(item => (
...
const LoginPage = ({ onLogin, showNotification, backendUrl }) => {
    ...
    const handleGoogleSuccess = async (token) => {
        ...
        try {
            ...
            onLogin(data); // Gửi cả object data
        } catch (err) {
...
export default function App() {
    ...
    const handleDailyLogin = async () => {
        ...
        try {
            ...
            const response = await fetch(`${backendUrl}/api/user/claim-daily`, {
                ...
                body: JSON.stringify({}), // Body rỗng vì userId đã có trong token
            });
            const result = await response.json();
            if (!result.success) throw new Error(result.message);
    ...
    const handleAdReward = async () => {
        ...
        try {
            ...
            const response = await fetch(`${backendUrl}/api/user/add-coins`, {
                ...
                body: JSON.stringify({ amountToAdd: 5 }),
            });
    ...
    const handleShareReferral = async (code) => {
        const shareData = {
            ...
            // Sửa lại URL để trỏ đến App ID mới
